...
describe('extractH1TitleFromContent', () => {
  ...
  it('preserves plain square brackets in the H1', () => {
    const content = '# [26Q2] HS-Hub MVP'
    expect(extractH1TitleFromContent(content)).toBe('[26Q2] HS-Hub MVP')
  })
...
describe('deriveDisplayTitleState', () => {
  ...
  it('keeps plain square brackets when deriving the display title from H1', () => {
    const content = '# [26Q2] HS-Hub MVP\n\nBody'
    expect(deriveDisplayTitleState({ content, filename: 'hs-hub-mvp.md' })).toEqual({
      title: '[26Q2] HS-Hub MVP',
      hasH1: true,
    })
  })
})
