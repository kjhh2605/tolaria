...
/// Also seeds HS-Hub-managed root type definitions used by repair/bootstrap flows.
...
mod tests {
    ...
    fn assert_refreshes_outdated_managed_agents(run: VaultOperation) {
        let outdated_agents = AGENTS_MD.replacen(
            ...
            "`type:` is the preferred type field. HS-Hub still understands legacy aliases such as `Is A`.",
            1,
        );
        ...
        assert!(!content.contains("HS-Hub still understands legacy aliases such as `Is A`."));
    }
    ...
    fn assert_required_agents_file_seeded(vault: &Path) {
        ...
        assert!(read_root_agents(vault).contains("HS-Hub Vault"));
    }
    ...
    fn test_seed_config_files_refreshes_stale_default_agents() {
        ...
        assert!(content.contains("HS-Hub reads notes recursively from all folders"));
    ...
    fn test_migrate_agents_md_idempotent_when_no_legacy() {
        ...
        assert!(root.contains("HS-Hub Vault"));
        assert_eq!(read_root_claude(&vault), CLAUDE_MD_SHIM);
    }
