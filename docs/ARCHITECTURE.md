...
## Hansung Study-Space Reservation

HS-Hub exposes a dedicated Korean-first `학습공간 예약` surface from the sidebar and command palette. The page owns search criteria, area-specific required fields, availability display, explicit live-booking confirmation, and post-success artifact actions while keeping all vault writes behind `App.tsx` callbacks. Team-member rows are shown only for areas that require them; coding lounge seminar rooms, Sangsang Park Plus small rooms, Sangsang Base, and library study spaces omit team members. Sangsang Park Plus collects affiliation, attendee count, and purpose for the school form’s required additional items; Sangsang Base collects all user names/student numbers and total attendee count. Library group-study rooms collect companion student numbers/names plus total attendee count, while the library meeting room and coworking room collect reservation reason plus total attendee count.

```mermaid
sequenceDiagram
    participant U as User
    participant UI as StudySpaceReservationPage
    participant API as studySpaceReservation.ts
    participant R as study_space_reservation.rs
    participant H as Hansung reservation adapter
    participant V as Active vault

    U->>UI: Select date/time/headcount/room and enter area-specific fields
    UI->>API: status/listSpaces/checkAvailability
    API->>R: Tauri invoke or mock handler
    R->>H: Read credentials from OS keychain, acquire transient token
    H-->>R: Availability or booking result
    R-->>API: Korean normalized result/error
    UI->>API: createReservation(confirm=true) after dialog
    R->>H: Perform live reservation and verify booking
    H-->>R: Success/failure, token discarded
    UI->>V: Save sanitized Markdown note under reservations/ on user action
    UI->>U: Offer local .ics download
```


HS-Hub also exposes a dedicated Korean-first `LMS 대시보드` surface from the sidebar and command palette. The dashboard is read-only in P0: it shows e-class connection state, overdue/today assignments, this-week deadlines, manual refresh/logout actions, and links back to original LMS pages. It does not submit or edit LMS content, create automatic notes, read grades/attendance, write Apple Calendar events, or run periodic polling.

The native boundary is `src-tauri/src/lms_dashboard.rs`; the renderer wrapper is `src/lib/lmsDashboard.ts`, with `src/hooks/useLmsDashboard.ts` owning page-open/resume/manual refresh behavior. The native layer invokes `src-tauri/resources/lms-hs-mcp-bridge.py`, which imports only Hs-MCP LMS client/tool modules from the bundled `study-space-python/` runtime. The command contract normalizes errors into Korean UI-safe messages, redacts sensitive details, caps returned courses/assignments, applies a subprocess timeout/output cap, and allows original links only when they target clean `https://learn.hansung.ac.kr` URLs. See [ADR-0128](./adr/0128-read-only-hansung-lms-dashboard.md).

The native boundary is `src-tauri/src/study_space_reservation.rs`; the renderer wrapper is `src/lib/studySpaceReservation.ts`. The native layer invokes `src-tauri/resources/study-space-hs-mcp-bridge.py`, which imports the bundled `hs_mcp` package and delegates to Hs-MCP `FacilityTools` where possible. Sangsang Park Plus reservation creation extends the verified Hs-MCP flow only at the form-submission boundary to populate the school form’s required `addItem1`/`addItem2`/`addItem3` values for affiliation, attendee count, and purpose. The adapter contract normalizes errors into Korean UI-safe messages and requires `confirm=true` before any live booking command runs. Credentials are sent only to the native login command; Hs-MCP stores facility session cookies in the OS credential store, while HS-Hub does not persist passwords in React state, vault files, settings, notes, logs, or calendar exports. For live booking verification, the bridge re-reads my reservations and selects the newest non-canceled matching reservation so stale canceled history cannot be reported as the newly created booking. See [ADR-0126](./adr/0126-hs-mcp-python-bridge-for-study-space.md).

Completed bookings can produce two sanitized artifacts:

- `reservations/<slug>.md` in the active vault, generated by `src/lib/studySpaceReservationArtifacts.ts` with `type: Reservation` frontmatter.
- A user-triggered `.ics` download for calendar import.

Both artifacts include only booking metadata, room/date/time, verification state, and entered team-member names/student numbers when that area requires members. Areas without member requirements record an explicit “no team-member info” row instead. They must not contain passwords, tokens, cookies, or raw MCP request/response payloads. See [ADR-0125](./adr/0125-hansung-study-space-reservation-artifacts.md).
