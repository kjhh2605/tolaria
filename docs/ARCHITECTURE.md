...
HS-Hub exposes a dedicated Korean-first `학습공간 예약` surface from the sidebar and command palette. The page owns search criteria, team-member rows, availability display, explicit live-booking confirmation, and post-success artifact actions while keeping all vault writes behind `App.tsx` callbacks.
...
sequenceDiagram
    ...
    U->>UI: Select date/time/headcount/room and enter members
...
Both artifacts include only booking metadata, room/date/time, verification state, and entered team-member names/student numbers. They must not contain passwords, tokens, cookies, or raw MCP request/response payloads. See [ADR-0125](./adr/0125-hansung-study-space-reservation-artifacts.md).
